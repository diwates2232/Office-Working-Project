...
function loadDevicesFromExcel() {
    const filePaths = [
        path.join(__dirname, "src/data/ArchiverData.xlsx"),
        path.join(__dirname, "src/data/CameraData.xlsx"),
        path.join(__dirname, "src/data/ControllerData.xlsx"),
        path.join(__dirname, "src/data/ServerData.xlsx")
    ];
    ...
    filePaths.forEach((filePath) => {
        ...
        data.forEach((row) => {
            if (row.IP) {
                deviceIPs.push(row.IP); // Assuming the column name is "IP"
            }
        });
    });
